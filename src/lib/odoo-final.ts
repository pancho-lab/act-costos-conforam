...
export class OdooFinalClient {
  ...
  private parseResponse(xml: string): any {
    ...
    // Check for array response with structs FIRST (has priority over simple int)
    if (xml.includes('<array>')) {
      ...
      if (arrayMatch) {
        ...
        // Split by struct boundaries to handle nested arrays properly
        const structSections = arrayContent.split('</struct></value>');
        console.log(`Found ${structSections.length - 1} potential structs`);
        
        for (let i = 0; i < structSections.length - 1; i++) {
          const section = structSections[i];
          const structMatch = section.match(/<value>\s*<struct>(.*)$/s);
          if (structMatch) {
            const structContent = structMatch[1];
            console.log(`Processing struct ${i + 1}:`, structContent.substring(0, 200) + '...');
            ...
            while ((memberMatch = memberRegex.exec(structContent)) !== null) {
              ...
              console.log(`  Processing member ${name}:`, valueContent.substring(0, 100));
              ...
              if (valueContent.includes('<string>')) {
                ...
                console.log(`    -> Boolean: ${item[name]}`);
              } else if (valueContent.includes('<array>')) {
                // Handle array values like parent_id [id, "name"]
                const nestedArrayMatch = valueContent.match(/<array><data>(.*?)<\/data><\/array>/s);
                if (nestedArrayMatch) {
                  const nestedValues: any[] = [];
                  const nestedValueRegex = /<value><(int|string|double)>(.*?)<\/\1><\/value>/gs;
                  let nestedMatch;
                  while ((nestedMatch = nestedValueRegex.exec(nestedArrayMatch[1])) !== null) {
                    const type = nestedMatch[1];
                    const val = nestedMatch[2];
                    if (type === 'int') {
                      nestedValues.push(parseInt(val));
                    } else if (type === 'double') {
                      nestedValues.push(parseFloat(val));
                    } else {
                      nestedValues.push(val);
                    }
                  }
                  item[name] = nestedValues;
                  console.log(`    -> Array: ${JSON.stringify(item[name])}`);
                } else {
                  item[name] = [];
                }
              } else {
    ...
    // Check for simple int response (like UID) - only if no array found
    if (xml.includes('<int>')) {
      const match = xml.match(/<int>(\d+)<\/int>/);
      if (match) {
        const result = parseInt(match[1]);
        console.log('Parsed simple INT:', result);
        return result;
      }
    }
    
    console.log('No parsing match found, returning xml');
    return xml;
  }
...
<methodCall>
  ...
  <params>
    ...
    if (typeof result === 'number' && result > 0) {
      this.uid = result;
      console.log(`✅ Authenticated! UID: ${result}`);
      return result;
    } else {
...
<methodCall>
  ...
  async getCategories(limit: number = 10): Promise<OdooCategory[]> {
    console.log(`📂 Getting ${limit} categories...`);
  ...
  async getProducts(domain: any[] = [], limit: number = 5): Promise<OdooProduct[]> {
    console.log(`📦 Getting ${limit} products...`);
...
// Factory singleton
let odooClient: OdooFinalClient | null = null;

export function getOdooClient(): OdooFinalClient {
  if (!odooClient) {
    const config = {
    ...
    odooClient = new OdooFinalClient(config);
  }
  return odooClient;
}